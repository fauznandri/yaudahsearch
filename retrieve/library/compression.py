...
class VBEPostings:
    ...
    def vb_encode_number(number):
        ...
        temp = []
        while True: 
            temp.insert(0, number%128) # hasil modulo dikumpulkan di list
            if number < 128:
                break
            else:
                number //= 128
                continue
        temp[-1] += 128 # menambah elemen pada index terakhir
        return bytearray(temp) # konversi list ke bytes
    ...
    def vb_encode(list_of_numbers):
        ...
        for i in list_of_numbers:
            res += VBEPostings.vb_encode_number(i)
        return res
    ...
    def encode(postings_list):
        ...
        bytes
            bytearray yang merepresentasikan urutan integer di postings_list
        """
        # Mengubah ke list of gaps
        postings_list_gaps = postings_list.copy()
        for i in range(1, len(postings_list_gaps)):
            postings_list_gaps[-i] -= postings_list_gaps[-i-1]
    ...
    def decode(encoded_postings_list):
        ...
        List[int]
            list of docIDs yang merupakan hasil decoding dari encoded_postings_list
        """
        res = VBEPostings.vb_decode(encoded_postings_list)
